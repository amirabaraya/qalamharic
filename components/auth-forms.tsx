...
export function LoginForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setStatus("Could not sign in. If the database is not configured yet, use the demo dashboard link.");
  }
...
export function SignupForm() {
  const router = useRouter();
  const [name, setName] = useState("Maya Tesfaye");
  ...
  return (
    <form className="mt-8 space-y-4" onSubmit={submit}>
      <label className="block">
        ...
        <span className="flex items-center gap-3 rounded-2xl border border-charcoal/10 bg-cream px-4 py-3 dark:border-cream/10 dark:bg-ink">
          <UserRound size={18} />
          <input className="w-full bg-transparent outline-none" type="text" value={name} onChange={(event) => setName(event.target.value)} />
        </span>
      </label>
      <label className="block">
        <span className="mb-2 block text-sm font-bold">Email</span>
        <span className="flex items-center gap-3 rounded-2xl border border-charcoal/10 bg-cream px-4 py-3 dark:border-cream/10 dark:bg-ink">
          <Mail size={18} />
          <input className="w-full bg-transparent outline-none" type="email" placeholder="maya@example.com" value={email} onChange={(event) => setEmail(event.target.value)} />
        </span>
      </label>
      <label className="block">
        <span className="mb-2 block text-sm font-bold">Password</span>
